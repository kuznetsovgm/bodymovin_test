...
exports.saveStickerToFile = exports.generateTextSticker = exports.LetterAnimationType = exports.ColorAnimationType = exports.TransformAnimationType = void 0;
...
function buildLetterSeed(letterIndex, charCode) {
...
async function generateTextSticker(opts) {
    const { text, transformAnimation = TransformAnimationType.None, colorAnimation = ColorAnimationType.None, letterAnimation = LetterAnimationType.None, fontSize = 72, fontPath = path.resolve(`./fonts/${FONT}`), width = 512, height = 512, frameRate = 60, duration = 180, } = opts;
    const fontObj = await promisify(opentype_js_1.default.load, fontPath);
    const { lines, finalFontSize } = wrapAndScaleText(text, fontObj, fontSize, width * 0.85, height * 0.85);
    const sticker = {
        tgs: 1,
        v: '5.5.2',
        fr: frameRate,
    ...
    switch (transformAnimation) {
        ...
        case TransformAnimationType.ShakeLoop: {
            const steps = 16, intensity = 20, pts = [], times = [];
            ...
            layer.ks.p = { a: 1, k: buildRawKeyframes(pts, times, true) };
            break;
        }
        ...
        case TransformAnimationType.Vibrate: {
            const steps = 30, intensity = 4, pts = [], times = [];
            for (let f = 0; f <= steps; f++) {
                const t = (f / steps) * duration;
                pts.push([
    ...
    const lettersGroup = convertTextToShapes(lines, fontObj, finalFontSize, colorAnimation, duration, letterAnimation, height);
    if (colorAnimation !== ColorAnimationType.Rainbow) {
        let cix = 1;
        if (colorAnimation === ColorAnimationType.CycleRGB)
            cix = 2;
        else if (colorAnimation === ColorAnimationType.Pulse)
            cix = 3;
...
function convertTextToShapes(lines, font, fontSize, colorAnimation, duration, letterAnimation, canvasHeight) {
    const group = {
        ...
        for (let i = 0; i < text.length; i++) {
            ...
            switch (letterAnimation) {
                ...
                case LetterAnimationType.TypingFall:
                    transform = createTypingFallLetterTransform(letterIndex, x, y, duration, canvasHeight);
                    break;
...
function createStaticTransform(index, x, y = 0) {
    return {
        cix: 200 + index,
...
function createVibrateTransform(index, x, y, duration) {
    const intensity = 2, steps = 30, pts = [], times = [];
    for (let f = 0; f <= steps; f++) {
        const t = (f / steps) * duration;
        pts.push([
    ...
    return {
        ...
        nm: `Transform_${index}`,
        hd: false,
        p: { a: 1, k: buildRawKeyframes(pts, times, true), ix: 2 },
        a: { a: 0, k: [0, 0], ix: 1 },
        s: { a: 0, k: [100, 100], ix: 3 },
        r: { a: 0, k: 0, ix: 6 },
        o: { a: 0, k: 100, ix: 7 },
...
function createTypingFallLetterTransform(index, x, finalY, duration, canvasHeight) {
...
function createWaveTransform(index, x, y, duration) {
    const amp = 12, steps = 40, pts = [], times = [], phase = index * 0.4;
    for (let f = 0; f <= steps; f++) {
        const t = (f / steps) * duration;
...
function createZigZagTransform(index, x, y, duration) {
    const spread = 35, steps = 48, pts = [], times = [], phase = index * Math.PI;
    for (let f = 0; f <= steps; f++) {
        const t = (f / steps) * duration;
        const angle = phase + (2 * Math.PI * t) / duration;
    ...
    return {
        ...
        p: { a: 0, k: [x, y], ix: 2 },
        a: { a: 0, k: [0, 0], ix: 1 },
        s: { a: 1, k: buildRawKeyframes(pts, times, true), ix: 3 },
        r: { a: 0, k: 0, ix: 6 },
        o: { a: 0, k: 100, ix: 7 },
        sk: { a: 0, k: 0, ix: 4 },
        sa: { a: 0, k: 0, ix: 5 },
...
function createWarpTransform(index, x, y, duration) {
...
function createFill(colorAnimation, duration, phase = 0, cix = 2) {
    switch (colorAnimation) {
        ...
        case ColorAnimationType.Rainbow: {
            ...
            return {
                ...
                c: {
                    a: 1,
                    k: buildRawKeyframes(pairs.map((p) => p.color), pairs.map((p) => p.time), true),
                    ix: 5,
                },
                o: { a: 0, k: 100 },
                r: 1,
...
async function saveStickerToFile(sticker, outPath) {
    ...
    const gz = await promisify(zlib.gzip, json);
