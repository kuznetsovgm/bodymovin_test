...
// New enum governing path (shape) morph animations independent from transform animations
export enum PathMorphAnimationType {
    None = 'none',
    Warp = 'warp', // basic corner warp
    WarpAiry = 'warpAiry', // current airy variant used before refactor
}
...
function wrapAndScaleText(
    ...
    const wrap = (fsz: number) => {
        ...
        // Для каждой явной строки применяем автоматический перенос по словам
...
// Removed legacy createWarpTransform: morph now handled via PathMorphAnimationType

function buildPathMorphKeyframes(
    bez: Bezier,
    fontSize: number,
    duration: number,
    ...
    const stateCount = 3; // could be parameterized later
    const states: Bezier[] = [];
    for (let s = 0; s < stateCount; s++) {
        const phase = (s / stateCount) * Math.PI * 2;
        let morphed: Bezier;
        if (morph === PathMorphAnimationType.WarpAiry) {
            morphed = warpCornerMorphAiry(bez, intensity, phase, seed);
        } else {
            morphed = warpCornerMorph(bez, intensity, phase, seed);
        }
        states.push(morphed);
    }
...
function createFill(
    ...
    switch (colorAnimation) {
        ...
        case ColorAnimationType.Static:
            return {
                cix,
                ty: ShapeType.Fill,
                c: {
