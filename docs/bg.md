...
- Реализованы базовые фоновые слои в пайплайне: `Solid`, `Frame`, `Stripes` (как отдельные `ShapeLayer` под текстом).
- Реализован базовый `knockoutBackground`: плашка + контуры букв в одном `ShapeLayer` с `FillRule.EvenOdd` (вырезается по заливке); поддерживаются layer‑level transform/color/pathMorph анимации, fill/ stroke для плашки. Пер‑буквенные анимации (в стиле `LetterAnimationDescriptor`) пока не применяются к дыркам.
- `GlyphPattern`/`TextLike` пока не реализованы (дескрипторы есть, но построение фигур пропускается).
